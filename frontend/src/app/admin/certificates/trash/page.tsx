...
import { useCertificates, useRestoreCertificate, useHardDeleteCertificate } from '../../../../presentation/hooks/useCertificates';
import { formatFriendlyDate } from '../../../../presentation/utils/dateUtils';

export default function AdminCertificatesTrashPage() {
  const router = useRouter();
  const { data, isLoading, isError } = useCertificates(true);
  const restoreMutation = useRestoreCertificate();
  const hardDeleteMutation = useHardDeleteCertificate();
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      toast.success('Certificado eliminado definitivamente');
  ...
  const handleRestore = async (id: string) => {
    try {
      ...
      toast.success('Certificado restaurado con éxito');
    } catch (err: any) {
  ...
  return (
    <div className="max-w-6xl mx-auto pb-10">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold" style={{ color: 'var(--text-primary)' }}>Papelera de Certificados</h1>
              ...
              {items.length === 0 ? (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="font-medium line-through opacity-60" style={{ color: 'var(--text-primary)' }}>{item.name}</div>
                    <div className="text-sm" style={{ color: 'var(--text-secondary)' }}>{item.issuingOrganization}</div>
                  </td>
      ...
      <ConfirmModal
        ...
        message="Esta acción no se puede deshacer. Este certificado será borrado definitivamente. ¿Continuar?"
