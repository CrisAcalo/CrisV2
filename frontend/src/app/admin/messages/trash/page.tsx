...
import { useMessages, useRestoreMessage, useHardDeleteMessage } from '../../../../presentation/hooks/useMessages';
import { formatFriendlyDate } from '../../../../presentation/utils/dateUtils';

export default function AdminCertificatesTrashPage() {
  const router = useRouter();
  const { data, isLoading, isError } = useMessages(true);
  const restoreMutation = useRestoreMessage();
  const hardDeleteMutation = useHardDeleteMessage();
  ...
  const handleRestore = async (id: string) => {
    try {
      ...
      toast.success('Mensaje restaurado con éxito');
    } catch (err: any) {
  ...
  return (
    <div className="max-w-6xl mx-auto pb-10">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold" style={{ color: 'var(--text-primary)' }}>Papelera de Mensajes</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead style={{ background: 'var(--surface-raised)', borderBottom: '1px solid var(--surface-border)' }}>
              <tr>
                <th className="px-6 py-4 font-semibold" style={{ color: 'var(--text-secondary)' }}>Remitente</th>
                <th className="px-6 py-4 font-semibold" style={{ color: 'var(--text-secondary)' }}>Asunto</th>
              ...
              {items.length === 0 ? (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="font-medium line-through opacity-60" style={{ color: 'var(--text-primary)' }}>{item.senderName}</div>
                    <div className="text-sm" style={{ color: 'var(--text-secondary)' }}>{item.senderEmail}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap" style={{ color: 'var(--text-secondary)' }}>
                    {item.subject}
                  </td>
                  <td className="text-xs truncate mt-0.5" style={{ color: 'var(--text-secondary)' }}>
      ...
      <ConfirmModal
        ...
        message="Esta acción no se puede deshacer. Este mensaje será borrado definitivamente. ¿Continuar?"
